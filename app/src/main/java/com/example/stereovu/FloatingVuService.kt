        ...
        thread {
            try {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q && captureIntent != null) {
                    ...
                    // Fallback: Visualizer módban mono, de Bluetooth-on is szólni fog ha internal source
                    // Itt egyszerű AudioRecord MIC-et használunk fallbacknek
                    val format = AudioFormat.Builder()
                        .setEncoding(AudioFormat.ENCODING_PCM_16BIT)
                        .setSampleRate(44100)
                        .setChannelMask(AudioFormat.CHANNEL_IN_STEREO)
                ...
                while (running) {
                    ...
                    if (read > 0) {
                        ...
                        var i = 0
                        while (i < read - 1) {
                            val l = buffer[i].toDouble() / Short.MAX_VALUE
                            val r = buffer[i+1].toDouble() / Short.MAX_VALUE
                            sumL += l*l
                            sumR += r*r
                            count++
                            i+=2
